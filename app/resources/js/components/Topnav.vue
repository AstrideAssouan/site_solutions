<script>
import logoutModal from '../components/modal/LogoutModal.vue';
import { getStore } from '../services/authService';

export default {
    components: {
        logoutModal,
    },
    data() {
        return {
            data: getStore(),
        }
    },
//     mounted() {
//         // Sélectionnez la balise span et le menu déroulant
// var spanElement = document.getElementById('userDropdown');
// var dropdownMenu = document.getElementById('userDropdownMenu');

// // Ajoutez un gestionnaire d'événements au span
// spanElement.addEventListener('click', function() {
//     // Vérifiez si le menu déroulant est actuellement caché ou affiché
//     if (dropdownMenu.style.display === 'none' || dropdownMenu.style.display === '') {
//         // Affichez le menu déroulant
//         dropdownMenu.style.display = 'block';
//     } else {
//         // Cachez le menu déroulant
//         dropdownMenu.style.display = 'none';
//     }
// });

// // Ajoutez un gestionnaire d'événements pour masquer le menu déroulant lorsqu'on clique en dehors de celui-ci
// document.addEventListener('click', function(event) {
//     if (!dropdownMenu.contains(event.target) && event.target !== spanElement) {
//         // Cachez le menu déroulant si l'événement a été déclenché en dehors du menu et de la balise span
//         dropdownMenu.style.display = 'none';
//     }
// });

//     },
    methods: {
        revenirAuSite() {
            // Redirige l'utilisateur vers l'accueil du site
            window.location.href = "/"; // Remplacez "/" par l'URL vers laquelle vous souhaitez rediriger l'utilisateur
        }
    }
}
</script>




<template>
    <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">

        <!-- <button id="sidebarToggleTop" class="btn btn-link d-md-none rounded-circle mr-3">
            <i class="fa fa-bars"></i>
        </button> -->

            <a href="#" class="btn" @click="revenirAuSite" >Revenir au site</a>

        <ul class="navbar-nav ml-auto">
            <!-- Nav Item - Alerts -->
            <li class="nav-item dropdown no-arrow mx-1">
                <a class="nav-link text-danger" href="#" id="alertsDropdown" data-bs-toggle="modal"
                    data-bs-target="#logoutModal">
                    <i class="fas fa-sign-out-alt"></i>
                </a>
            </li>

            <div class="topbar-divider d-none d-sm-block"></div>

            <!-- Nav Item - User Information -->
            
            <div class="user-area dropdown float-right">
                <li class="nav-item">
                    <a class="nav-link" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true"
                        aria-expanded="true">
                        <span id="userDropdown" class="mr-2 d-none d-lg-inline text-gray-600 small">{{ data.nom }} {{ data.prenom }}</span>
                        <img class="img-profile rounded-circle"
                            src="data:image/png;base64,"> 
                    </a>
                
                    <!-- <div class="dropdown-menu dropdown-menu-right user-dd animated show"  id="userDropdownMenu">
                        <a class="dropdown-item" href="/back-office/user/edit/{{ data.id }}"><i class="ti-user m-r-5 m-l-5"></i> Mon compte</a>
                        <a class="dropdown-item" href="#" data-bs-toggle="modal"
                        data-bs-target="#logoutModal"><i class="fa fa-power-off m-r-5 m-l-5"></i> Déconnexion</a>
                    </div> -->
                </li>

                
            </div>

        </ul>

    </nav>

    <logoutModal />
</template>



<style scoped>
.btn {
  color: white;
  background-color: #00c9a7; /* Couleur bleue comme exemple, vous pouvez ajuster la couleur selon vos préférences */
  padding: 10px 20px;
  text-decoration: none;
  border-radius: 5px;
  /* Autres styles CSS selon vos besoins */
}
</style>